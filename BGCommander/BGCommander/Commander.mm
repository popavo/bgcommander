...
Command& Commander::command(const StringRef& _n, bool addIfMissing) {
  return Command::AppCommand == _n ? Command::AppCommand : *Command::AppCommand.command(_n, addIfMissing);
}
Commander::iterator Commander::removeCommand(const Command& _c) { return Command::AppCommand.removeCommand(_c); }
Command& Commander::addCommand(const Command& __c) {
  Command::AppCommand.addCommand(__c);
  return *find(__c.name);
}
...
Commander::search_depth Commander::search(const StringRef& name, NSInteger maxDepth) {
  NSInteger depth = 0;
  return Command::AppCommand.search(name, maxDepth, depth);
}
Commander::const_search_depth Commander::search(const StringRef& name, NSInteger maxDepth) const {
  NSInteger depth = 0;
  return Command::AppCommand.search(name, maxDepth, depth);
}
Commander::search_depth Commander::search(const Command& cmd, NSInteger maxDepth) {
  NSInteger depth = 0;
  return Command::AppCommand.search(cmd, maxDepth, depth);
}
Commander::const_search_depth Commander::search(const Command& cmd, NSInteger maxDepth) const {
  NSInteger depth = 0;
  return Command::AppCommand.search(cmd, maxDepth, depth);
}
...
void Commander::resetAllParentRefs() { Command::AppCommand.resetParentRefs(); }
