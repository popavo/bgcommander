...
+ (id) new {
  ...
  if (![helper isKindOfClass:[GBOptionsHelper class]])
    return helper;
...
- (NSString*)versionString {
  ...
  if (version)
    [output appendFormat:@": version %@", version];
  if (build)
    [output appendFormat:@" (build %@)", build];
  return [output copy];
}
...
- (NSString*)valuesStringFromSettings:(GBSettings*)settings includeArguments:(BOOL)includeArgs {
  ...
  [self enumerateOptions:^(GBOption* definition, BOOL* stop) {
      if (![blockSelf isPrint:definition])
        return;
      ...
      GB_UPDATE_MAX_LENGTH(longOption)[columns addObject : longOption];
      ...
      [settings enumerateSettings:^(GBSettings* settings, BOOL* stop) {
          ...
          GB_UPDATE_MAX_LENGTH(columnData)[columns addObject : columnData];
      }];
