...
typedef NS_ENUM(int, CommanderExits) { NO_EXIT = INT32_MIN, };
...
class BGRef {
 public:
  ...
  BGRef& operator=(const _Tp rs) {
    set(rs);
    return *this;
  }
  BGRef& operator=(const BGRef& rs) {
    copy_assign(rs);
    return *this;
  }
  BGRef& operator=(BGRef&& rs) {
    move_assign(std::move(rs));
    rs.zero();
    return *this;
  }
 ...
 private:
  void copy_assign(const BGRef& rs) {
    getter = rs.getter;
    setter = rs.setter;
    set(rs.fRef);
  }
  void move_assign(BGRef&& rs) {
    getter = std::move(rs.getter);
    setter = std::move(rs.setter);
    move_set(std::move(rs.fRef));
  }
  ...
  void set(const _Tp& rs) {
    if (setter != NULL)
      fRef = setter(fRef, rs);
    else
      fRef = rs;
  }
  void move_set(_Tp&& rs) {
    if (setter != NULL)
      fRef = std::move(setter(fRef, rs));
    else
      fRef = std::move(rs);
  }
