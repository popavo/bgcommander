...
#define OBJC_STD_HASH(__type)                                 \
  ...
  struct std::hash<__type> {                                  \
    std::size_t operator()(__type __v) { return [__v hash]; } \
  }
...
template <>
struct hash<id> {
  size_t operator()(const id& __v) { return [__v hash]; }
};
...
struct hash<bg::StringRef> {
  size_t operator()(const bg::StringRef& __v) const { return __v.hash(); }
};

template <>
struct hash<bg::Command> {
  size_t operator()(const bg::Command& __v) const { return __v.hash(); }
};
