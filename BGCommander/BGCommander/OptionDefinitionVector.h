...
class OptionDefinition {
 public:
  ...
  OptionDefinition(char _so = 0, StringRef _lo = @"", StringRef _d = @"", GBOptionFlags _f = 0)
      : shortOption(_so), longOption(_lo), description(_d), flags(_f) {}

  OptionDefinition(const OptionDefinition& rs) { _copy(rs); }
  OptionDefinition(OptionDefinition&& rs) {
    _move(std::move(rs));
    rs.clear();
  }

  OptionDefinition& operator=(const OptionDefinition& rs) {
    _copy(rs);
    return *this;
  }
  OptionDefinition& operator=(OptionDefinition&& rs) {
    _move(std::move(rs));
    rs.clear();
    return *this;
  }
 ...
 private:
  void _copy(const OptionDefinition& rs) {
    shortOption = rs.shortOption;
    longOption = rs.longOption;
    description = rs.description;
    flags = rs.flags;
  }
  void _move(OptionDefinition&& rs) {
    shortOption = std::move(rs.shortOption);
    longOption = std::move(rs.longOption);
    description = std::move(rs.description);
    flags = std::move(rs.flags);
  }
};
